...
:- type tex == model.tex.
:- type vertex ---> vertex(vert_index::int, tex_index::int, normal_index::int).
...
% A material that can mostly express a wavefront mtllib
:- type material --->
    material(diffuse::color.color,
        ambient::color.color,
        specular::color.color,
        ambient_texture::string, % You'll probably be fine only use ambient tex.
...
:- instance model(shape) where [
    (putpoint(Point, shape(Points, Tex, Nmls, Faces, Mtl), shape(Out, Tex, Nmls, Faces, Mtl)) :-
        list.append(Points, [Point|[]], Out)),
    (puttex(TexCoord, shape(Points, Tex, Nmls, Faces, Mtl), shape(Points, Out, Nmls, Faces, Mtl)) :-
        list.append(Tex, [TexCoord|[]], Out)),
    (putnormal(Normal, shape(Points, Tex, Nmls, Faces, Mtl), shape(Points, Tex, Out, Faces, Mtl)) :-
        list.append(Nmls, [Normal|[]], Out)),
    putface(Face, shape(Points, Tex, Nmls, Faces, Mtl), shape(Points, Tex, Nmls, [Face | Faces], Mtl))
].

init_material = material(color.white, color.white, color.white, "", "").

init_shape = shape([], [], [], [], init_material).
...
accumulate_face(!Parser, FaceIn, FaceOut) :-
    ...
    ( get(!Parser, Char), ( Char = '\n' ; Char = '\v' ; Char = '#') ->
        FaceOut = FaceIn
    ;
        list.append(FaceIn, [vertex(V, T, N) |[]], FaceMid),
        GetInt(!Parser, V+1),
        ( get(!Parser, '/') ->
            GetInt(!Parser, T+1),
